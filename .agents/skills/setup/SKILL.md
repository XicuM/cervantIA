---
name: setup
description: Establishes project configuration & style.
metadata: { "openclaw": { "emoji": "⚙️" } }
---
# Setup Skill

1. **Interview**: Ask missing `wiki/style_guide.md` details:
   - Language
   - POV (1st, 3rd limited/omniscient)
   - Tense (past/present)
   - Tone
   - Audience
   - Chapter structure/naming (`ch-01.md`)
   - Scene breaks
   - Vocab constraints/profanity
   - Genre
2. **Config**: Update `wiki/style_guide.md` templates with answers.
3. **Schema**: Update `AGENTS.md` if schema/conventions change.
4. **Log**: Append `## [YYYY-MM-DD HH:MM] setup | <Summary>` to `wiki/log.md`.
5. **Reconfig**: If user changes preferences later, read `style_guide.md` and update only changed fields.
