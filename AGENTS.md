...
## Skills
- `setup` — Establish project configuration and style.
- `query` — Gather context (timeline, characters, locations).
- `search` — Backend tool: Perform keyword searches across wiki files.
- `write` — Draft in `manuscript/` per style guide and wiki.
- `ingest` — Sync wiki with new entities from draft.
- `lint` — Validate consistency across wiki/manuscript.
...
## Logs
Append to `logs/[YYYY-MM].md`: `## [YYYY-MM-DD HH:MM] skill_name | Short description`
